...
class FullNode extends Base {
  ...
  async getBlockByHeight({ height }) {
    ...
    return blocks.reduce((bestBlock, block) => {
      if (!bestBlock) {
        return block;
      }
      const bestBlockTotalIters = new BigNumber(bestBlock.reward_chain_block.total_iters);
      const blockTotalIters = new BigNumber(block.reward_chain_block.total_iters);

      return bestBlockTotalIters.isLessThan(blockTotalIters) ? bestBlock : blockTotalIters;
    }, null);
  }

  async getBlocks({ startBlockHeight, endBlockHeight, excludeReorged = true }) {
    const res = await this.connection.send(new Message({
      command: 'get_blocks',
      data: {
  ...
  async getAverageNetworkSpaceForHeight(height) {
    return this.getNetworkSpaceForHeights({ startHeight: Math.max(1, height - 4608), endHeight: height });
  }
